...
(transient-define-prefix composer-dispatch ()
  "Invoke a Composer command."
  ["Composer"
   [("i" "Install" composer-install-dispatch)
    ("r" "Require" composer-require-dispatch)
    ("s" "Run script" composer-run-script)
    ("d" "Dump autoload" composer-dump-autoload)]]
  ["Open file"
   [("l" "Lock file" composer-view-lock-file)
    ("j" "Json file" composer-find-json-file)]])

(transient-define-argument composer--transient-prefer-install-argument ()
  "Composer `--prefer-install' argument."
  :description "Prefer install"
  :argument "--prefer-install="
  :allow-empty nil
  :always-read t
  :key "-pi"
  :class 'transient-option
  :init-value (lambda (obj) (oset obj value "auto"))
  :choices '("auto" "source" "dist"))

(transient-define-argument composer--transient-no-interaction-argument ()
  "Composer `--no-interation' argument."
  :description "No interaction"
  :argument "--no-interaction"
  :key "-n")

(transient-define-argument composer--transient-optimize-autoloader-argument ()
  "Composer `--optimize-autoloader' argument."
  :description "Optimize autoloader"
  :argument "--optimize-autoloader"
  :key "-o")

(defvar composer--transient-package nil "Composer transient package")

(transient-define-infix composer--transient-package-infix ()
  "Composer packages argument."
  :prompt "Select a package: "
  :class 'transient-lisp-variable
  :transient t
  :variable 'composer--transient-package)

(transient-define-prefix composer-install-dispatch ()
  "Invoke composer install."
  ["Arguments"
   [(composer--transient-prefer-install-argument)
    (composer--transient-no-interaction-argument)
    (composer--transient-optimize-autoloader-argument)]]
  ["Actions"
   [("i" "Install" composer-install-command)]])

(transient-define-prefix composer-require-dispatch ()
  "Invoke composer require"
  ["Arguments"
   [("-pkg" "Package" composer--transient-package-infix)
    (composer--transient-prefer-install-argument)
    (composer--transient-no-interaction-argument)
    (composer--transient-optimize-autoloader-argument)]]
  ["Actions"
   [("r" "Require" composer-require-command)]])

(defun composer-install-command ()
  (interactive)
  (apply 'composer--command-async-execute "install"
         (transient-args 'composer-install-dispatch)))

(defun composer-require-command ()
  (interactive)
  (let ((args (transient-args 'composer-require-dispatch)))
    (let ((options (if composer--transient-package
                       (append args (list (format "%s" composer--transient-package)))
                     args)))
         (apply 'composer--command-async-execute "require" options))))
